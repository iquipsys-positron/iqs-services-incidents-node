...
export interface IIncidentsPersistence extends IGetter<IncidentV1, string>, IWriter<IncidentV1, string> {
    ...
    getCountByFilter(correlationId: string, filter: FilterParams, 
    ...
    create(correlationId: string, item: IncidentV1, 
        callback: (err: any, item: IncidentV1) => void): void;

    closeById(correlationId: string, id: string, userId: string, 
        resolutioId: string, resolution: string, 
        callback: (err: any, item: IncidentV1) => void): void;

    deleteById(correlationId: string, id: string,
        callback: (err: any, item: IncidentV1) => void): void;

    deleteExpired(correlationId: string, expireTime: Date,
